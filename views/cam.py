...
chord_dict = {0: 'down', 1: 'left', 2: 'right', 3: 'up'}
chordsave=[]
...
def main():
    ...
    while cap.isOpened():
        ...
        if len(contours) > 0:
            ...
            print(str(chord_dict[pred_class]))
            #chordsave.append(chord_dict[pred_class])
...
print(chordsave)
showchord=[]
for i in range(len(chordsave)):
    if i==0 :
        showchord.append(chordsave[i])
    elif chordsave[i]!=chordsave[i-1]:
        showchord.append(chordsave[i])
    elif chordsave[i]==chordsave[i-1]:
        showchord.append(' ')
