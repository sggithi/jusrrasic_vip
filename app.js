...
app.get("/game/:name", async (req, res) => {
  ...
  const inventory = await Inventory.find().where({
    player: player,
    have: true,
  });
  ...
  if (action === "query") {
    ...
    if (_eventType) {
      if (_eventType === "battle") {
        ...
        while (true) {
          if (turn <= 10 && player.HP / player.maxHP > 0.2) {
            ...
            if (dinoHP <= 0) {
              ...
              const attack = "attack";
              const armor = "armor";
              const attItem = await Inventory.find({
                player: player,
                have: true,
                type: attack,
              });
              const armorItem = await Inventory.find({
                player: player,
                have: true,
                type: armor,
              });
        ...
        let haveItem = await Inventory.findOne({
          player: player,
          itemId: _itemNum,
        });
        if (!haveItem) {
